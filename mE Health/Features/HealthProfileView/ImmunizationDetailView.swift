...
struct ImmunizationDetailView: View {
   ...
   @State private var savedFiles: [SavedMedia] = []
    private var practitionerFiles: [SavedMedia] {
        savedFiles.filter { $0.categoryId == immune.id }
    }
    ...
    private var get_file_name: String {
        "\(immune.vaccineCodeDisplay)_\(immune.formattedOccurrenceDate)"
    }
...
//                        .padding(.horizontal,12)
                ...
                .sheet(isPresented: $showShare) {
                    ShareSheet(activityItems: [generateImmunizationShareText()])
                }
        ...
        //                        savedFiles.append(saved)
                ...
                .onAppear {
                    ...
                    if selectedCategory == nil {
                        selectedCategory = UploadCategory(name: "Immunizations", id: immune.id)
                    }
    ...
    private func findEncounter() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            if let encounter = visitsVM.visitData.first(where: { $0.id == immune.encounterId }) {
    ...
    func generateImmunizationShareText() -> String {
        let vaccineName = immune.vaccineCodeDisplay
        let immunizationId = immune.id
        let status = immune.status.capitalized
        let occurrenceDate = immune.formattedOccurrenceDate
        let patientName = "\(userProfileData?.first_name ?? "") \(userProfileData?.last_name ?? "")"
        
        return """
        Immunization Details

        Here's my immunization info from mEinstein — your digital healthcare assistant!
